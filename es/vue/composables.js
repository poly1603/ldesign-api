 ...
 * ***********************************
 */
import { inject, getCurrentInstance, ref, computed, onUnmounted, watch } from 'vue';
import { SYSTEM_API_METHODS } from '../types/index.js';
import { ApiErrorFactory, ApiError } from '../utils/ApiError.js';
import { API_ENGINE_INJECTION_KEY } from './plugin.js';
...
function useApi() {
  const injectedEngine = inject(API_ENGINE_INJECTION_KEY);
  if (injectedEngine) {
...
function useSystemApi() {
  return {
     ...
     * 获取验证码
     */
    getCaptcha: (options = {}) => useApiCall(SYSTEM_API_METHODS.GET_CAPTCHA, options),
    /**
     * 用户登录
     */
    login: (options = {}) => useApiCall(SYSTEM_API_METHODS.LOGIN, options),
    /**
     * 用户登出
     */
    logout: (options = {}) => useApiCall(SYSTEM_API_METHODS.LOGOUT, options),
    /**
     * 获取用户信息
     */
    getUserInfo: (options = {}) => useApiCall(SYSTEM_API_METHODS.GET_USER_INFO, options),
    /**
     * 更新用户信息
     */
    updateUserInfo: (options = {}) => useApiCall(SYSTEM_API_METHODS.UPDATE_USER_INFO, options),
    /**
     * 获取系统菜单
     */
    getMenus: (options = {}) => useApiCall(SYSTEM_API_METHODS.GET_MENUS, options),
    /**
     * 获取用户权限
     */
    getPermissions: (options = {}) => useApiCall(SYSTEM_API_METHODS.GET_PERMISSIONS, options),
    /**
     * 刷新令牌
     */
    refreshToken: (options = {}) => useApiCall(SYSTEM_API_METHODS.REFRESH_TOKEN, options),
    /**
     * 修改密码
     */
    changePassword: (options = {}) => useApiCall(SYSTEM_API_METHODS.CHANGE_PASSWORD, options),
    /**
     * 获取系统配置
     */
    getSystemConfig: (options = {}) => useApiCall(SYSTEM_API_METHODS.GET_SYSTEM_CONFIG, options)
  };
}
...
export { useApi, useApiCall, useApiCleanup, useApiPolling, useBatchApiCall, useInfiniteApi, useMutation, usePaginatedApi, useSystemApi };
