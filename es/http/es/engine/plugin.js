...
import { HttpPlugin as s$1 } from '../vue/plugin.js';
  ...
  return { name: i2, version: l2, dependencies: [], async install(o3) {
    try {
      if (o3 && "object" == typeof o3.config && "function" == typeof o3.provide) {
        ...
        await (async () => {
          const o4 = p || (() => {
            const n3 = s(r2.adapter);
            return new w({ ...r2, ...g }, n3);
        ...
        const s$2 = o3.engine || o3;
        if (!s$2 || "function" != typeof s$2.getApp) return;
        const u = async () => {
          const o4 = s$2.getApp();
          if (!o4) throw new Error("Vue app not found. Make sure the engine has created a Vue app before installing HTTP plugin.");
          const u2 = p || (() => {
            const n2 = s(r2.adapter);
            return new w({ ...r2, ...g }, n2);
          })();
          o4.use(s$1, { client: u2, globalConfig: g || r2, globalProperty: a, ...c }), s$2.http ? s$2.http.setInstance(u2) : s$2.httpClient = u2, s$2.logger && s$2.logger.info(`${i2} plugin installed successfully`, { version: l2, clientType: u2.constructor.name });
        };
        s$2.getApp() ? (s$2.logger && s$2.logger.info("[HTTP Plugin] Vue app found, installing immediately"), await u()) : (s$2.logger && s$2.logger.info("[HTTP Plugin] Vue app not found, registering event listener"), await new Promise((e2, t2) => {
          s$2.events.once("app:created", async () => {
            try {
              s$2.logger && s$2.logger.info("[HTTP Plugin] app:created event received, installing now"), await u(), e2();
            } catch (e3) {
              s$2.logger && s$2.logger.error("[HTTP Plugin] Failed to install after app creation:", e3), t2(e3);
            }
          });
        })), s$2.logger && s$2.logger.info(`${i2} plugin registered, waiting for Vue app creation...`);
      }
    } catch (e2) {
      throw e2;
    }
...
export { o as createHttpEnginePlugin };
