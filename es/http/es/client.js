 ...
 * ***********************************
 */
import { InterceptorManagerImpl as t$2 } from './interceptors/manager.js';
import { generateId as K } from './utils/index.js';
import { CacheManager as s } from './utils/cache.js';
import { globalCancelManager as c } from './utils/cancel.js';
import { ConcurrencyManager as s$1 } from './utils/concurrency.js';
import { DownloadProgressCalculator as e, getFilenameFromResponse as t$4, getFilenameFromURL as n, getMimeTypeFromFilename as a$1, saveFileToLocal as o$2 } from './utils/download.js';
import { RetryManager as a } from './utils/error.js';
import { RequestMonitor as t } from './utils/monitor.js';
import { RequestPool as t$1 } from './utils/pool.js';
import { PriorityQueue as s$2, determinePriority as o } from './utils/priority.js';
import { validateFile as t$3, ProgressCalculator as r, createUploadFormData as o$1 } from './utils/upload.js';
...
class w {
  constructor(t2 = {}, a2) {
    ...
    this.adapter = a2, this.retryManager = new a(t2.retry), this.cancelManager = c, this.cacheManager = new s(t2.cache), this.concurrencyManager = new s$1(t2.concurrency), this.monitor = new t(t2.monitor), this.priorityQueue = new s$2(t2.priorityQueue), this.requestPool = new t$1(t2.connectionPool), this.interceptors = { request: new t$2(), response: new t$2(), error: new t$2() };
  }
  async request(e2) {
    this.checkDestroyed();
    const r2 = this.optimizedMergeConfig(e2), s2 = K();
  ...
  async uploadSingleFile(e2, t2, r2) {
    t$3(t2, r2);
    const s2 = Date.now(), o2 = new r(), a2 = { method: "POST", url: e2, data: o$1(t2, r2), headers: { ...r2.headers || {} }, ...r2 || {}, onUploadProgress: r2.onProgress ? (e3) => {
  ...
  async uploadMultipleFiles(e2, t2, r2) {
    t2.forEach((e3) => t$3(e3, r2));
    const s2 = Date.now(), o2 = new r(), a2 = new FormData(), n2 = r2.fileField || "files";
    t2.forEach((e3, t3) => {
      a2.append(`${n2}[${t3}]`, e3);
  ...
  async download(e2, t2 = {}) {
    this.checkDestroyed();
    const r2 = Date.now(), s2 = new e(), o2 = { method: "GET", url: e2, responseType: "blob", ...t2 || {}, onDownloadProgress: t2.onProgress ? (e3) => {
      const r3 = s2.calculate(e3.loaded, e3.total || 0, t2.filename);
      t2.onProgress?.(r3);
    } : void 0 }, h2 = await this.request(o2);
    let l2 = t2.filename;
    l2 || (l2 = t$4(h2.headers) || n(h2.config.url || e2) || "download");
    const d2 = h2.data?.type || a$1(l2);
    let p2;
    return false !== t2.autoSave && typeof window < "u" && (o$2(h2.data, l2), p2 = URL.createObjectURL(h2.data)), { data: h2.data, filename: l2, size: h2.data.size, type: d2, duration: Date.now() - r2, url: p2 };
  }
  getPerformanceStats() {
