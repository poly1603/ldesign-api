 ...
 */
var t, e;
(e = t || (t = {}))[e.CRITICAL = 0] = "CRITICAL", e[e.HIGH = 1] = "HIGH", e[e.NORMAL = 2] = "NORMAL", e[e.LOW = 3] = "LOW", e[e.IDLE = 4] = "IDLE";
class s {
  constructor(e2 = {}) {
    ...
    for (const e3 of Object.values(t)) "number" == typeof e3 && this.queue.set(e3, []);
    this.config?.priorityBoost && this.startPriorityBoost();
  }
  async enqueue(e2, s2, i2 = t.NORMAL) {
  ...
  getNextItem() {
    for (const e2 of [t.CRITICAL, t.HIGH, t.NORMAL, t.LOW, t.IDLE]) {
  ...
  performPriorityBoost(e2) {
    ...
    for (const [i2, o2] of this.queue.entries()) {
      if (i2 === t.CRITICAL || 0 === o2.length) continue;
      const r = this.config?.boostInterval;
      for (let n = 0; n < o2.length; n++) {
        const u = o2[n];
        if (e2 - u.timestamp > r) {
          const e3 = Math.max(t.CRITICAL, i2 - 1);
...
function o(e2) {
  ...
  const s2 = e2.url || "";
  return s2.includes("/auth") || s2.includes("/login") || s2.includes("/token") ? t.CRITICAL : s2.includes("/api") ? t.HIGH : s2.match(/\.(jpg|jpeg|png|gif|css|js)$/i) ? t.LOW : s2.includes("/analytics") || s2.includes("/log") ? t.IDLE : t.NORMAL;
}

export { t as Priority, s as PriorityQueue, o as determinePriority };
