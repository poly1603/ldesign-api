 ...
 */
var e, t;
(t = e || (e = {})).IMMEDIATE = "immediate", t.EXPONENTIAL = "exponential", t.LINEAR = "linear", t.FIXED = "fixed", t.NONE = "none";
class a {
  constructor(e2 = {}) {
  ...
  shouldRetry(t2, a2) {
    if (a2 >= this.config?.maxRetries) return { shouldRetry: false, delay: 0, strategy: e.NONE, reason: `\u5DF2\u8FBE\u5230\u6700\u5927\u91CD\u8BD5\u6B21\u6570 ${this.config?.maxRetries}` };
    if (this.config?.customDecision) {
      const e2 = this.config?.customDecision(t2, a2);
      if (e2) return e2;
    }
    if (this.config?.checkNetworkStatus && this.isOffline()) return { shouldRetry: false, delay: 0, strategy: e.NONE, reason: "\u5F53\u524D\u7F51\u7EDC\u79BB\u7EBF" };
    const s2 = t2.response?.status;
    return s2 ? this.config?.nonRetryableStatusCodes.includes(s2) ? { shouldRetry: false, delay: 0, strategy: e.NONE, reason: `\u72B6\u6001\u7801 ${s2} \u4E0D\u53EF\u91CD\u8BD5\uFF08\u5BA2\u6237\u7AEF\u9519\u8BEF\uFF09` } : this.config?.retryableStatusCodes.includes(s2) ? this.createRetryDecision(true, a2, this.getStrategyForStatusCode(s2), `\u72B6\u6001\u7801 ${s2} \u53EF\u91CD\u8BD5`) : s2 >= 500 && s2 < 600 ? this.createRetryDecision(true, a2, e.EXPONENTIAL, `\u670D\u52A1\u5668\u9519\u8BEF ${s2}\uFF0C\u4F7F\u7528\u6307\u6570\u9000\u907F\u91CD\u8BD5`) : { shouldRetry: false, delay: 0, strategy: e.NONE, reason: `\u72B6\u6001\u7801 ${s2} \u4E0D\u5728\u91CD\u8BD5\u8303\u56F4\u5185` } : this.createRetryDecision(true, a2, e.EXPONENTIAL, "\u7F51\u7EDC\u9519\u8BEF\uFF0C\u4F7F\u7528\u6307\u6570\u9000\u907F\u91CD\u8BD5");
  }
...
export { e as RetryStrategy, a as SmartRetryManager };
