...
exports.SpanType = void 0; var a; exports.SpanStatus = void 0;
(a = exports.SpanType || (exports.SpanType = {})).HTTP = "http", a.DATABASE = "database", a.CACHE = "cache", a.CUSTOM = "custom", (function(t2) {
...
class r {
  ...
  startTrace(t2, a2 = exports.SpanType.HTTP) {
    if (!this.shouldSample()) return new c();
    const r2 = this.generateTraceId(), o2 = { spanId: this.generateSpanId(), traceId: r2, name: t2, type: a2, startTime: Date.now(), status: exports.SpanStatus.PENDING, tags: [], logs: [] }, d2 = { traceId: r2, currentSpan: o2, spans: [o2], metadata: {} };
  ...
  generateTraceId() {
    return `trace-${index.generateId()}-${Date.now()}`;
  }
  generateSpanId() {
    return `span-${index.generateId()}`;
  }
  getConfig() {
    return this.config;
  }
...
class n {
  ...
  startSpan(a2, r2 = exports.SpanType.CUSTOM) {
    const n2 = { spanId: `span-${index.generateId()}`, parentSpanId: this.context.currentSpan?.spanId, traceId: this.context.traceId, name: a2, type: r2, startTime: Date.now(), status: exports.SpanStatus.PENDING, tags: [], logs: [] };
  ...
  setError(t2) {
    return this.rootSpan.status = exports.SpanStatus.ERROR, this.rootSpan.error = { message: t2.message, stack: t2.stack }, this;
  }
  finish(t2 = exports.SpanStatus.SUCCESS) {
    const e2 = Date.now();
...
class o {
  ...
  setError(t2) {
    return this.span.status = exports.SpanStatus.ERROR, this.span.error = { message: t2.message, stack: t2.stack }, this;
  }
  finish(t2 = exports.SpanStatus.SUCCESS) {
    const e2 = Date.now();
