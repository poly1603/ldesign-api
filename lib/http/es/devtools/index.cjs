...
class o {
  ...
  attach(e2) {
    ...
    this.interceptorIds = { request: o2, response: s2, error: -1 }, this.config?.logToConsole && logger.logger.info("DevTools attached to HTTP client");
  }
  detach() {
    !this.client || !this.interceptorIds || (this.client.removeRequestInterceptor(this.interceptorIds.request), this.client.removeResponseInterceptor(this.interceptorIds.response), this.client = null, this.interceptorIds = null, this.config?.logToConsole && logger.logger.info("DevTools detached from HTTP client"));
  }
  onRequestStart(e2) {
    const o2 = this.generateId(), s2 = { id: o2, timestamp: Date.now(), config: e2, status: "pending" };
    e2.__devtools_id__ = o2, this.addRecord(s2), this.config?.logToConsole && (logger.logger.group(`\u27A1\uFE0F ${e2.method?.toUpperCase()} ${e2.url}`), logger.logger.debug("Config:", e2), logger.logger.groupEnd());
  }
  onRequestSuccess(e2) {
    ...
    if (s2 && (s2.duration = Date.now() - s2.timestamp, s2.response = e2, s2.status = "success", this.config?.logToConsole)) {
      ...
      logger.logger.group(`${r2} ${e2.config.method?.toUpperCase()} ${e2.config.url} (${s2.duration}ms)`), logger.logger.debug("Status:", e2.status), logger.logger.debug("Data:", e2.data), o3 && logger.logger.warn(`\u6162\u8BF7\u6C42\u8B66\u544A: \u8017\u65F6 ${s2.duration}ms`), logger.logger.groupEnd();
    }
  }
  onRequestError(e2) {
    const o2 = e2.config.__devtools_id__;
    if (!o2) return;
    const s2 = this.findRecord(o2);
    s2 && (s2.duration = Date.now() - s2.timestamp, s2.error = e2, s2.status = "error", this.config?.logToConsole && (logger.logger.group(`\u274C ${e2.config?.method?.toUpperCase()} ${e2.config?.url} (${s2.duration}ms)`), logger.logger.error("Error:", e2.message), logger.logger.error("Details:", e2), logger.logger.groupEnd()));
  }
  addRecord(e2) {
  ...
  clear() {
    this.records = [], this.config?.logToConsole && logger.logger.info("DevTools records cleared");
  }
  export() {
  ...
  download() {
    if (typeof window > "u") return void logger.logger.warn("Download is only available in browser");
    const e2 = this.export(), o2 = new Blob([e2], { type: "application/json" }), s2 = URL.createObjectURL(o2), r2 = document.createElement("a");
    r2.href = s2, r2.download = `http-devtools-${Date.now()}.json`, r2.click(), URL.revokeObjectURL(s2), logger.logger.info("DevTools data exported");
  }
  printStats() {
    this.getStats();
  }
...
exports.HttpDevTools = o;
exports.globalDevTools = r;
