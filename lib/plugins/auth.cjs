  ...
  return {
    ...
    install(engine) {
      ...
      const errMw = async (err) => {
        ...
        if (engine.hasMethod?.(index.SYSTEM_API_METHODS.REFRESH_TOKEN)) {
          try {
            const result = await engine.call(index.SYSTEM_API_METHODS.REFRESH_TOKEN);
...
exports.authMiddlewaresPlugin = authMiddlewaresPlugin;
exports.createAuthMiddlewaresPlugin = createAuthMiddlewaresPlugin;
