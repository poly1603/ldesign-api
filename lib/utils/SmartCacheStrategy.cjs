...
class SmartCacheStrategy {
  ...
  getSuggestedTTL(key, baseTTL) {
    ...
    const priority = this.priorityMap.get(key) || exports.CachePriority.NORMAL;
    switch (priority) {
      case exports.CachePriority.CRITICAL:
        return baseTTL * 10;
      // 10倍TTL
      case exports.CachePriority.HIGH:
        return baseTTL * this.config?.hotDataTTLMultiplier;
      case exports.CachePriority.LOW:
   ...
   */
  getPriority(key) {
    return this.priorityMap.get(key) || exports.CachePriority.NORMAL;
  }
  /**
   * 设置缓存优先级
   */
  ...
  adjustPriority(key, stats) {
    const currentPriority = this.priorityMap.get(key) || exports.CachePriority.NORMAL;
    if (currentPriority === exports.CachePriority.CRITICAL) {
      return;
    }
    const accessScore = stats.accessCount;
    const frequencyScore = stats.averageInterval > 0 ? 1e3 / stats.averageInterval : 0;
    let newPriority = exports.CachePriority.NORMAL;
    if (accessScore >= 10 && frequencyScore > 0.1) {
      newPriority = exports.CachePriority.HIGH;
    } else if (accessScore < this.config?.minAccessThreshold) {
      newPriority = exports.CachePriority.LOW;
    }
...
//# sourceMappingURL=SmartCacheStrategy.cjs.map
