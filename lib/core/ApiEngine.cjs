...
var RequestQueue = require('../utils/RequestQueue.cjs');
var version = require('../version.cjs');
var factory = require('../http/es/factory.cjs');
...
class ApiEngineImpl {
  constructor(config = {}) {
    ...
    this.httpClient = factory.createHttpClient(this.config.http);
    this.cacheManager = new CacheManager.CacheManager(this.config.cache);
    this.debounceManager = new DebounceManager.DebounceManagerImpl();
    this.deduplicationManager = new DeduplicationManager.DeduplicationManagerImpl();
    this.middlewareCache = new LRUCache.LRUCache({
      maxSize: 100,
      defaultTTL: 36e5,
      // 1小时
      enabled: true
    ...
    if (this.config?.queue?.enabled) {
      ...
      this.requestQueueManager = new RequestQueue.RequestQueueManager(q);
    }
    this.errorReporter = ErrorReporter.getGlobalErrorReporter();
      ...
      const performOnce = async () => {
        ...
        if (useQueue) {
          if (!this.requestQueueManager) {
            this.requestQueueManager = new RequestQueue.RequestQueueManager({
              enabled: true,
...
exports.ApiEngineImpl = ApiEngineImpl;
/*! End of @ldesign/api | Powered by @ldesign/builder */
//# sourceMappingURL=ApiEngine.cjs.map
